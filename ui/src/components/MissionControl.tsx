...
export default function MissionControl() {
  return (
    <div className="flex flex-col gap-base h-full w-full">
      {/*  Top KPI Row  */}
      ...
      {/*  Main Content Area  */}
      <div className="grid grid-cols-3 gap-base flex-grow min-h-0">
        ...
        <div className="col-span-1 glass-panel rounded-lg flex flex-col overflow-hidden">
          ...
          <div className="p-sm flex-grow overflow-y-auto flex flex-col gap-sm">
            {[
              { name: "Alex Chen", role: "Frontend Lead", status: "online", img: "1" },
              { name: "Sarah Miller", role: "DevOps Engineer", status: "busy", img: "2" },
              { name: "David Kim", role: "Backend Architect", status: "offline", img: "3" },
              { name: "Elena Rodriguez", role: "Security Spec.", status: "online", img: "4" }
            ].map((member, i) => (
              <div key={i} className="flex items-center gap-sm p-sm rounded hover:bg-white/5 transition-colors cursor-pointer">
                <div className="relative">
                  <div className="w-8 h-8 rounded-full bg-surface-variant flex items-center justify-center text-outline-variant font-bold">
                    {member.name.charAt(0)}
                  </div>
                  <div className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border border-surface ${member.status === 'online' ? 'bg-tertiary' : member.status === 'busy' ? 'bg-error' : 'bg-outline-variant'}`}></div>
                </div>
                <div className="flex flex-col">
